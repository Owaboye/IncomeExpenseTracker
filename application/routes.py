...
def dashboard():
    ...
    ).filter(IncomeExpTracker.type == 'Income').scalar()
    ...
    income_by_Category = db.session.query(
        ...
        db.func.sum(IncomeExpTracker.amount).label("Amount")
    ).filter(IncomeExpTracker.type == 'Income').group_by(IncomeExpTracker.category).all()

    # Expenses by Category
    exp_by_Category = db.session.query(
        IncomeExpTracker.category,
        db.func.sum(IncomeExpTracker.amount).label("Amount")
    ).filter(IncomeExpTracker.type == 'Expenses').group_by(IncomeExpTracker.category).all()

    monthly_expenses = db.session.query(
        db.func.strftime('%Y-%m', IncomeExpTracker.created_at).label("month"),
        db.func.sum(IncomeExpTracker.amount).label("total")
    ).filter(IncomeExpTracker.type == 'Expenses').group_by("month").all()

    
    monthly_income = db.session.query(
        db.func.strftime('%Y-%m', IncomeExpTracker.created_at).label("month"),
        db.func.sum(IncomeExpTracker.amount).label("total")
    ).filter(IncomeExpTracker.type == 'Income').group_by("month").all()
